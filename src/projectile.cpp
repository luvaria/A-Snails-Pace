...
ECS::Entity SlugProjectile::createProjectile(vec2 position, vec2 velocity) {
	...
	// Create rendering primitives
	std::string key = "slugProjectile";
	ShadedMesh& resource = cache_resource(key);
	if (resource.mesh.vertices.size() == 0)
	{
		resource.mesh.loadFromOBJFile(mesh_path("slugprojectile.obj"));
		RenderSystem::createColoredMesh(resource, "slugProjectile");
	}

	std::string key_min = "minslugprojectile";
	ShadedMesh& resource_min = cache_resource(key_min);
	if (resource_min.mesh.vertices.size() == 0)
	{
		resource_min.mesh.loadFromMinOBJFile(mesh_path("minslugprojectile.obj"));
	}
	...
	// Create and (empty) Slug Projectile component to be able to refer to all projectiles
